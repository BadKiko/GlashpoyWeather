package com.glashpoy.weather.ui.components
...
import com.glashpoy.weather.domain.hourStringInTwelveHourFormat
...
fun DailyForecastCard(
    ...
    Card(modifier = modifier) {
        ...
        LazyRow(
            ...
            itemsIndexed(hourlyForecasts.reversed().take(10)) { index, day ->
                DailyForecastItem(
                    dateTime = getDatesInRussian()[index],
                    iconResId = day.weatherIconResId,
...
fun getDatesInRussian(): List<String> {
    val locale = Locale("ru", "RU")
    val currentDate = LocalDate.now()
    val formatter = DateTimeFormatter.ofPattern("EEEE", locale)
    return (0..10).map { currentDate.plusDays(it.toLong()).format(formatter) }
}